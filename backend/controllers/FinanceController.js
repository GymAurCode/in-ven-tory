...
class FinanceController {
  ...
  static async getFinancialOverview(req, res) {
    try {
      ...
      // Only count income from sales (type 'auto') for dashboard metrics
      let incomeQuery = "SELECT SUM(amount) as total FROM income WHERE type = 'auto'";
      let expenseQuery = 'SELECT SUM(amount) as total FROM expenses';
      let salesQuery = 'SELECT COUNT(*) as count, SUM(quantity) as total_quantity FROM sales';
      let productsQuery = 'SELECT COUNT(*) as count FROM products';
      const params = [];
      const conditions = [];
      ...
      if (conditions.length > 0) {
        ...
        expenseQuery += ' WHERE ' + conditions.join(' AND ');
      ...
      const productsStats = db.prepare(productsQuery).get();

      const income = totalIncome.total || 0;
      const expenses = totalExpenses.total || 0;
      const netProfit = income - expenses;
      const donation = netProfit > 0 ? netProfit * 0.02 : 0; // 2% donation only if profit is positive
      ...
      // Get distinct sold products count
      const distinctSoldProducts = db.prepare('SELECT COUNT(DISTINCT product_id) as count FROM sales').get();

      // Get total products with stock > 0
      const stockProductsCount = db.prepare('SELECT COUNT(*) as count FROM products WHERE quantity > 0').get();
      ...
      res.json({
        ...
        partnerBShare,
        // Dashboard metrics
        totalProducts: productsStats.count,
        stockProductsCount: stockProductsCount.count, // Products with stock > 0
        totalSold: salesStats.total_quantity || 0,
        totalSales: salesStats.count,
        distinctSoldProducts: distinctSoldProducts.count
      });
    } catch (error) {
